...
typedef enum {MENU_PRINCIPAL, INSTRUCOES, JOGO, SAIR, MENU_JOGADORES} GameState;
GameState gameState;
...
bool moverCasa(int ci, int cj, int quant){
    ...
    if(cj>5 && pj<5){
        ...
        addMensagem("jogador %d nao pode avancar %d casas a partir daqui.", jogador, quant);
        return false;
...
int main(int argc, char argv[]) {
    ...
    //struct State* stateabc = newState(pecas,'A',6,0);
    //printStateTree(stateabc);
    //SetConsoleCP(20127);
...
GameState jogo(){
    ...
    while(true){
        ...
        int ci, cj;
        JOGA:
        if(pecas[j*2][4] > 0 && pecas[j*2][4]+pecasS[j] == 7){
